...
本版本为英文 9 键键盘新增可选的「复古 9 键」多击输入，默认关闭，同时修复多击间隔无效的问题。多击是英文 9 键上的一个选项，开关关闭时与原版完全一致，中文输入、候选、学习、手写、主题和分页行为均保持不变。

### Added

- 英文 9 键键盘新增可选的「复古 9 键」：打开后在同一数字键上连续点击，按点击次数选择该键分组的第 N 个字母，单击 `2` 出 `a`，时间窗内再点出 `b`、`c`，超过字母数后循环回第一个字母
- 在「输入设置 → 英文」新增「复古 9 键」开关，默认关闭；「多击间隔」挂在它下面，默认 600 ms，可选 300、400、500、600、800 和 1000 ms，开关关闭时置灰不可点但仍保留已选值
- 等待窗口没结束前，待定字母以 composing 文本呈现，编辑器为其绘制下划线；窗口结束、换键或结束输入时下划线消失，字母保留

### Changed

- 多击是英文 9 键上的一个选项，不新增键盘布局：`ime_en_9key.xml` 只把 IME 类指向兼容类，`string_id`、键盘组和标签保持不变，键盘选择页仍只有「英文 26 键」和「英文 9 键」两个英文条目
- 开关关闭时兼容类把 `handle`、联想、自动纠错和标点候选全部交回原版 `English9KeyIme`，原版英文 9 键、中文九宫格、全键盘、手写和密码键盘的定义与输入行为保持不变
- 打开后该键盘不联想，连击循环直接替换 composing 区域里的字母，不再先删除再重新提交，因此不再依赖「本轮内光标未移动」这一前提

### Fixed

- 修复「多击间隔」钳位分支写反导致窗口恒为 2000 ms：取值合法时本应跳过赋值，原实现把下限写成 `if-lt`、上限写成 `if-gt`，任何取值都会先被改成 100 再被改成 2000
...
Pixel 10 Pro / Android 16 上先用隔离审计包跑通全部门禁，再以正式签名、非 Debug 的体验包覆盖安装并实测：
...
- 验收前后英文用户词典与个人词典的文件内容未变，覆盖安装后默认输入法与 Rime 同步配置保持不变

### Build

- 新增 `scripts/verify_english_t9_multitap.py`，检查开关关闭时对 `handle`、联想、自动纠错和候选的父类回退、原生 composing 路径、间隔钳位方向、键盘选择页没有独立多击布局、原版英文 9 键定义与软键未被改写，以及旧设置页和 Compose 设置的一致性

## [2.1.0] - 2026-09-14

本版本新增 Rime 用户词典双向同步，并重做用户词库状态的展示。手动与自动同步共用同一套执行、恢复和安全校验逻辑，Google 拼音原有的输入、候选、学习、手写、主题和分页行为保持不变，用户词典格式与此前的备份、导入语义也没有改变。

### Added

- 在「词典与备份」中新增 Rime 同步区域，通过 Storage Access Framework 授权同步目录，配置本设备名称与 Rime 用户词典文件名，即可在 Google 拼音与 Rime 各设备之间同步多字词条的新增、删除和重新添加
- 新增自动同步后台任务，使用系统 JobScheduler 的持久周期作业，默认间隔 24 小时，可选 6 小时、12 小时、24 小时、2 天、3 天和 7 天，开启时明确告知包含删除行为
- 新增词库健康状态指示，以颜色和文字共同显示「可正常读取」「需要注意」和「暂时无法确认」，颜色不作为唯一提示
- 新增展开详情，显示「上次成功同步：双方共有 N 条，仅保留在 Rime 的有 M 条」，并在自动开关开启时于折叠状态显示上次成功时间

### Changed

- 手动「立即同步」与自动同步共用同一执行入口，正常路径一次点击完成，只在首次接受处理规则或涉及删除时保留确认
- 同步只处理多字词条的存在与删除，不换算历史词频。暂时无法由 Google 拼音直接接收的词条自动保留在 Rime，不逐批询问，不修改原编码，也不转写英文用户词典
- 更换同步目录时在确认数据归属后延续设备身份、盐、基线和保留标记，不再要求删除本设备文件夹
- 词库是否可读取改由词库接口本身判断，移除此前用原生 `getDictionaryCount()` 权重之和冒充词条总数的显示
- Rime 同步区域位于备份与导入之后、自定义短语之前，并移除其中重复的当前同步状态行

### Fixed

- 修复更换存储位置后同步关系被当作新配置的问题。根 URI 改变不再重新生成设备身份和盐，已有快照身份不匹配时拒绝保存该位置，缺少本设备快照时不把整份基线解释成删除
- 修复展开与收起详情时的界面抖动，复查期间保留上次结果并在等尺寸按钮图标位显示进度
- 修复自动同步完成后已打开的设置页不能及时刷新状态的问题
...
- 新增 Rime 同步协议、状态存储、SAF、Native Bridge 和协调器主机回归，以及 Compose Rime 快照状态测试
- 扩展构建与生成流程，加入 Rime Smali 可复现生成检查、Manifest 组件注册和 API 21 能力开关
- 最终 APK 通过 API 31、33、34、35、36 静态门禁、6,633 个旧公开资源 ID 核对、Compose 运行时结构检查、v1/v2/v3 签名与 16 KiB ZIP alignment
...
- Pixel 10 Pro / Android 16 隔离审计覆盖 Telegram X 等密码输入框与普通输入框之间的切换、区域内普通点击、区域外松手分页、展开候选分页、左右边界、快速 fling 和现有轻拖阈值，未发现回弹、误选、按键状态卡住或分页目标变化。审计进程保持存活，crash buffer 中没有匹配审计包的条目
...
- 无插桩基线确认原问题不是 CPU、GPU、layout/draw、GC 或热节流：显示能力 120 Hz 时，旧 IME Surface 调度为 60 Hz、活动帧间隔约 16.67 ms、FrameTimeline jank 为 0、Thermal Status 为 0。release-like non-debuggable 包在 Pixel 10 Pro / Android 16 上确认约 120 fps、现代 jank 为 0、Missed Vsync 为 0、Slow UI thread 为 0，静止后 120 Hz override 消失
...
- Pixel 10 Pro / Android 16 上使用 Bitwarden `2026.7.0` 验收 5 个凭据加 1 个 Vault 入口，并确认随机选择凭据可正常填充。实际建议数量由 Provider 决定，6 是输入法接受的总上限，不保证每个 Provider 都返回 6 项
