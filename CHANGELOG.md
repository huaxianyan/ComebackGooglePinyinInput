...
- 英文 9 键键盘新增可选的「多击选字」：打开后在同一数字键上连续点击，按点击次数选择该键分组的第 N 个字母，单击 `2` 出 `a`，时间窗内再点出 `b`、`c`，超过字母数后循环回第一个字母
- 在「输入设置 → 英文」新增「多击选字」开关，默认关闭；「多击间隔」挂在它下面，默认 600 ms，可选 300、400、500、600、800 和 1000 ms，开关关闭时置灰不可点但仍保留已选值
