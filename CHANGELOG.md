...
- 新增可独立选择的英文 T9 多击键盘：在原生键盘布局列表中与「英文 26 键」「英文 9 键」并列，单击数字键出该键第一个字母，时间窗内连续点击依次选后续字母，超过字母数后循环，该键盘不联想
- 在「输入设置 → 英文」新增「多击间隔」，默认 600 ms，可选 300、400、500、600、800 和 1000 ms，旧 Preference 与 API 35+ Compose 设置共用同一 key 与默认值
- 等待窗口没结束前，待定字母以 composing 文本呈现，编辑器为其绘制下划线；窗口结束、换键或结束输入时下划线消失，字母保留

### Changed

- 新键盘通过新增英文 IME 与键盘定义实现，原版英文 9 键、中文九宫格、全键盘、手写和密码键盘的定义与输入路径保持不变
- 新键盘复用原版九宫格软键与 keymapping，按键音与振动沿用原生触摸管线
- 连击循环改为直接替换 composing 区域里的字母，不再先删除再重新提交，因此不再依赖「本轮内光标未移动」这一前提

### Build

- 新增 `scripts/verify_english_t9_multitap.py`，检查新 IME 注册、原版英文 9 键未被改写、原生 composing 路径与间隔钳位方向，以及旧设置页和 Compose 设置的一致性
