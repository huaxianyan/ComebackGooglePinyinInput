...
两次回弹都进入保留的 `settle` 分支，8 次成功翻页都进入 `fling`，没有出现「进入 fling 后仍回弹」。因此这轮证据排除了目标页方向、页码钳制和 fling 分支执行错误。对这两次短手势，释放速度没有达到原生最小 fling 速度，同时位移未超过原生 50% settle 阈值，返回当前页符合旧语义。当时没有足够证据修改阈值。
...
- 3 次回弹全部是 `settle + target_initial_same`
- 17 次成功翻页全部是 `target_changed`
- 没有 `target_clamped_same`
- 没有 fling 后回弹。

这证明目标页方向、边界钳制和 Scroller 后续状态不是本轮缺陷。问题被收敛为：日常短滑偶尔未达到 fling 速度，同时旧 settle 要求拖过页面宽度的 50%。

第一版位移探针错误地把 `v3` 当成绝对页位置，其全部位移分类已作废。`v3` 实际是相对于 `Llo.a` 基准页的局部比例，修正公式为：

```text
abs((Llo 基准页 + 局部比例) - 当前选中页)
```

修正采样产生 22 组日志。用户说明开头有两次误触，唯一零误差对齐是排除最前两组：两次误触均为 `d_lt_12_5 + settle + target_initial_same`。剩余 20 次与用户观察完全一致，其中两次明确短滑回弹均位于 `d_12_5_25`。这形成了可验证的行为边界：
...
25% 阈值无法覆盖这两次回弹。12.5% 是当前证据支持的最保守候选值，而不是根据手感随意选择的常量。

## 12.5% settle A/B 结果

A/B 实现只修改 `PageableRecentSubCategorySoftKeyListHolderView` 的非 fling 目标选择：
...
原生 paging touch slop、横纵方向竞争、VelocityTracker、系统 minimum fling velocity、V34 fling、目标页边界钳制、Scroller、高刷生命周期、点击取消和无障碍路径均保持不变。候选 pager 和其他 `lk` 使用者继续使用原生 50% settle。

Pixel 10 Pro / Android 16 的隔离 A/B 验证结果：

- 日常快速短滑：20/20 翻页，回弹 0/20
- 普通项目点击：误翻页 0/10
- 小于明确翻页意图的轻拖：误翻页 0/10，10 次位移均小于 12.5% 且目标保持当前页。

轻拖仍可能提交表情，因为小位移可以保留原生项目点击语义。该现象不是 settle 目标修改引入的翻页回归，不应在本任务中修改 touch slop、项目提交或外层点击取消链路。

因此正式候选采用精确子类范围内的对称 12.5% settle，并删除所有速度、时序、目标页和位移诊断调用。正式补丁只保留目标计算辅助类与静态门禁。
