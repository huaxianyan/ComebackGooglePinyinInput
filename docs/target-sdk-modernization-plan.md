...
- V22 的系统控件完整可见且可交互，但用户确认主题 surface 出现异常，且与 target 28 对比底部空间明显过宽。这修正了此前把 `mandatorySystemGestures=84` 当作视觉控制 frame 的推断：现场 Taskbar 的实际 navigation window/frame 高度是 63 px，84 px 是不可由应用排除的强制手势识别区域，不要求所有视觉内容避让。V20 控件被遮挡的原因是 frame z-order，不是 63 px 导航高度不足。V23 因此保留 index-0 主题 frame、移除 `bringToFront()`，但恢复只按 `navigationBars()` 的 63 px 动态间距；这样与 target 28 的紧凑空间一致，同时系统控件在主题 frame 上层完整绘制。仍不硬编码任何像素。
