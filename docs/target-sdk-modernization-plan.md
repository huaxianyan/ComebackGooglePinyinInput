...
- V26 不引用 framework 内部类名、隐藏 ID或反射：通过公开 `ViewGroup` API取得 DecorView 最后一个 direct child，验证它是非空 ViewGroup 且已有正高度后，使用其实际 `getHeight()` 作为动态 bottom gap，并把主题 frame 以 index 0 插入这个容器内部；原有系统导航 View 保持在其上层。若结构不满足条件，则回退到公开 WindowMetrics 的 `navigationBars()`。这应在当前手势模式动态得到 126 px，同时解决 target 28 垂直对齐、完整控件和整块主题色；三键/旋转也读取各自实际容器高度，不硬编码像素。
