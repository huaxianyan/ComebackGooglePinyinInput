...
- V21 中左右系统控件仍可交互但完全不可见，同时底部主题连续且应用 Insets 正确，反向证明 84 px 几何已正确，剩余问题是绘制层级：decor-root 主题 frame 在创建和每次更新后调用 `bringToFront()`，覆盖了 framework/SystemUI 控件的可见像素，却不拥有其系统触摸处理。V22 将主题 frame 以 index 0 插入 decor root，并禁止 `bringToFront()`；InputView 已通过 bottom margin 结束在该 frame 上方，因此 frame 在底部仍可见，而 framework 后续/既有 decor children 和系统控制保持在它上层。该修复只改变 z-order，不改变 IME source、动态 84 px 间距或主题来源。
