...
- V29 隔离 Debug 将 decor tree 深度扩展到 8，增加 visibility/alpha/willNotDraw 等非内容状态，并在完成日志后仅对导航容器最后一个原有 direct child 试验 `setWillNotDraw(true)`。该 View 在 V25 中对应 background=null 的 framework navigation View；若灰色消失而按钮子 View 仍正常，即可证明灰色来自它的 `onDraw()`/transitions，而不是背景 Drawable、Window scrim 或主题 frame。必须分别在三按钮和手势模式验证后才允许转成 release-like 修复。
