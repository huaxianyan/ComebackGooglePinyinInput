...
- V30 Debug 在 `configureImeWindow(InputMethodService)` 已取得真实 Window 时保存该公开 Window 引用，最终 Insets listener 直接用它重申 `setNavigationBarContrastEnforced(false)`；不再通过 View Context 猜测 Service，也撤销 V28 的透明色实验和 V29 的 `setWillNotDraw()` 实验。先在三按钮验证灰层，再切换手势验证无回归；只有两种模式都通过才清理 Debug 并形成 release-like 修复。
