...
> **当前状态：手动同步已完成真实词库闭环，自动同步纳入本次版本。** 自动同步尚未实现，已确认默认 24 小时、可调整为 6 小时至 7 天，开启时告知包含删除，只处理当前已有文件，见 [自动同步设计](rime-auto-sync-design.md)。Bridge 继续作为同步目录中的一个稳定 Rime 设备，合并现有设备快照，转换双方新增和删除，只发布自己的快照。
...
用户单独授权在独立测试包中进行编码对照，结束后清理。本轮通过本机 ADB，在非 Debug 的 `rimeautosyncaudit` 包中复用已有产物和生产 `GoogleNativeDictionaryBridge.apply()`／`read()`，没有连接 SAF 目录、访问正式包词库或启用测试输入法。只使用 7 个固定对照项，不重新批量导入用户词库。

每项均从空词典开始。成功项完成生产入口的持久化与完整复核，再读取并确认编码和词语组合原样保留，随后通过生产删除入口恢复空词典。失败项确认异常为 `RejectedEntriesException`、拒绝数为 1，词典仍为空。结果如下，具体词条正文不写入公开文档：

| 对照类型 | 结果 |
|---|---|
| 普通汉字词语与正确拼音 | 写入及持久化复核成功 |
| 同一词语，换成非标准音节 | 原生插入接口返回失败 |
| 同一词语，换成合法但读音错配的音节 | 写入及持久化复核成功，错配组合原样读回 |
| 多字词语与完整分音节拼音 | 写入及持久化复核成功 |
| 同一多字词语，换成自定义缩写 | 原生插入接口返回失败 |
| 英文词面与原英文编码 | 原生插入接口返回失败 |
| 同一英文词面，换成中文拼音音节 | 写入及持久化复核成功 |

这组结果反驳了「当前桥接使用的写入路径强制要求编码与词语实际读音一致」的假设，也证明词面并非必须全部为汉字。它支持编码音节能否被当前中文拼音写入形态接受是重要因素，但没有穷尽原生词典的音节表、长度或其他约束，不能据此认定只要编码合法就一定成功，更不能把任何插入失败都自动归类为编码不兼容。

本轮只验证原生写入、保存与重载后的记录，不代表已验证错配编码或英文词面在键盘上的候选显示、提交和学习。没有新增整份词库写入、进程冷重启或词频实验，也没有修改同步策略。对照结束后已卸载隔离包和探针，删除本轮本地临时探针 APK，没有创建手机公共测试文件。原默认输入法和正式包进程保持不变。
