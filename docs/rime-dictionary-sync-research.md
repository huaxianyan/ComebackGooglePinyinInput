...
修复已从固定原始 APK 完整重建，生成 65 个 Rime Smali 文件，通过协议测试和 API 31、33、34、35、36 静态门禁，并验证 v1/v2/v3 签名和 16 KiB ZIP alignment。资源表与两个现代 DEX 均与前一候选逐字节一致，因此既有 6,633 个资源 ID 的证据仍适用。新 APK SHA-256 为 `24b221389072d5b35ca415baf18ab50c8ff166ba4475c99fffd75f219c65260f`。实际写入和后续零变更闭环尚待用户确认后执行，本轮仅完成名称修复与真实目录预览。
