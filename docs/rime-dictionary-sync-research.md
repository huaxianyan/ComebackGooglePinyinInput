...
- **写入方刷新边界尚未统一：** 对 `70d84c7` 构建产物的交错路径静态审阅发现，`UserDictImportTask.doInBackground()` 虽然持有共享锁，但在导入、持久化和关闭 accessor 后仍调用异步 `notifyMutableDictionaryDataChanged()`，随后解锁。`SaveDictionaryTask` 同样仍异步通知，且关闭 accessor 位于通知调用之后。因此，共享锁串行化了文件写入，却没有保证下一位调用者取得锁时已加载的数据也是最新版本。在主线程刷新尚未运行时，紧随其后的 Bridge 读取可能看到旧数据，后续写入可能覆盖刚导入的结果。这是源码确认的时序窗口，不是已经复现的数据丢失事故。Rime 单独新增／删除通过不能作为全部写入方交错安全的证明。下一步须让相关写入方在释放共享锁前完成数据刷新，并保持输入会话通知只在主线程执行，继续复用现有刷新入口，不增加锁、队列或定时重试。
