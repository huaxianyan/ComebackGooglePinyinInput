...
- [x] 修复数据库名称兼容：去掉完整 `.userdb.txt` 后缀，标准 peer 使用 `pinyin_simp`。只迁移本机目录内带有效 Bridge 标记和 UUID 的旧快照，不重建 Profile 或 baseline。Pixel 隔离包已通过「标准空 peer＋旧名称空 Bridge → Preview → 发布」真实入口验证，发布名称正确、身份保持、peer 字节不变。已有非空 Profile 和未完成事务迁移仍需在最终回归中核对
...
- [x] 最新隔离候选 `2.0.10-rime-copywriting-audit.1` 完成 APK 身份、最终 DEX 和运行时契约核对并安装到 Pixel。三个 DEX 与已验收产物逐字节一致，助手未启动、启用或配置新包，旧审计数据与原默认输入法保持不变
- [ ] 用户完成最新候选的首次引导、日常输入及设置体验确认，不把技术门禁和单项真机通过扩展为所有布局、视觉体验和长期学习均已验收
...
- [ ] 上述问题解决后，构建最终隔离候选包，统一完成静态门禁与整合回归，再决定合并和发布
