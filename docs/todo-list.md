...
- [ ] 隔离验证修正版原生候选视觉：ART 类加载无 `VerifyError`、关闭按钮按退格/九键光标/语音参考键窗口坐标对齐真实右列中心、右线直接使用 Candidate 原生 divider 且左线清除自身 tint/filter 后共享同一已主题化 Drawable、无左侧人工 reserve、200dp 内保持原生字号并省略、关闭及展开箭头恢复
