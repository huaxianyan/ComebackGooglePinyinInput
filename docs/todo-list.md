...
- [ ] 优化用户词典状态展示并加入 Rime 信息：分组呈现本机词库与同步状态、最近成功同步、自动同步及仅保留在 Rime 的数量，替换「滚动副本」「恢复旁路」「落盘」等难懂术语，文件细节按需查看。复用现有状态来源，保持只读、按需刷新，核对英文统计与导出数量不一致的口径，不为显示数量扫描整份词库。待实际使用验收结束后集中处理，详见 [状态展示设计](dictionary-health-status-design.md#待实施词库与-rime-同步状态总览)
