...
- [ ] 隔离验证最终剪贴板候选细节：ART 类加载无 `VerifyError`、关闭按钮保持真实右列中心、两侧原生主题 divider 不回退、无左侧人工 reserve、文字前 18dp Material paste 图标继承候选文字颜色且与文字共同限制在 200dp/END ellipsis 内、回收普通候选时不残留图标或间距、关闭及展开箭头恢复
