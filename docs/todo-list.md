...
- [ ] 页面层级 V1 已建立：导航首页、输入/中文/英文/模糊拼音、键盘四个子页、词典与其他占位页均使用可保存的显式 route stack，纯逻辑 push/pop/异常恢复已有单元测试；待真机验收各入口、系统/工具栏 Back、配置重建和跨页依赖后，再将仍在单体 `SettingsScreen.kt` 中的页面与共享组件按文件拆分，不在结构提交中混入新设置语义
