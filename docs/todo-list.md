...
- [x] 英文 9 键新增可选的「复古 9 键」：同一数字键连续点击按次数取字母，超过字母数后循环，等待窗口内以 composing 文本显示下划线
- [x] 不新增键盘布局：`ime_en_9key.xml` 只改 IME 类，选项关闭时把 `handle`、联想、自动纠错和标点候选全部交回原版 `English9KeyIme`
- [x] 新增默认关闭的「复古 9 键」开关和依赖它的「多击间隔」，旧 Preference 与 Compose 共用同一 key、默认值和父子关系
- [x] 修复「多击间隔」钳位分支写反导致窗口恒为 2000 ms
- [x] 静态门禁、Compose 单元测试、从原始 APK 的完整重建与双构建一致性通过
- [x] 真机确认打开后 2 至 9 按 a→b→c 循环、下划线随窗口出现与消失、4 字母键 `7` 与 `9`、大小写和按键音振动
- [x] 真机确认关闭开关后英文 9 键的联想、自动纠错、标点候选和中英文输入完全回到原版，且键盘选择页仍只有两个英文条目
...
- [ ] 完成 Rime 用户词典手动双向同步的真实设备和大词典验收：Bridge 核心、Primary DEX 异步门面、独立 SAF 配置、预览与删除确认、恢复和重建状态，以及「自动备份」下方的 Compose「Rime 同步」类别均已实现。API 36 隔离模拟器已验证 SAF 持久授权、SQLite 建库、Native 新增与事务性重建删除、未删除条目保留、进程重启持久化，以及 `PLANNED` 中断后的幂等恢复；测试同时发现并修复 `ExternalStorageProvider` 临时文件自动补 `.txt` 和精确单条 Native 删除失败。10 万级 translated-ARM64 基准已完成，首次预览 47.4 秒、执行 173.3 秒，零增量预览 103.4 秒，观测内存峰值最高约 426 MiB。`v4` 已验证 Native 持久化后静默少项会保留 `PLANNED` 且不发布 Bridge 快照，恢复只补齐缺失项并最终得到 10 万项零变更闭环。`v5` 已实现在本地 Profile 丢失后从唯一、有效且身份规范的既有 Bridge 快照恢复稳定 `user_id`，并通过 API 36 的 4 KiB 隔离模拟器 SAF 预览验收。Pixel 10 Pro 已完成 97,808 项真实 SAF 预览，首次执行和一次恢复都被 Native 持久化复核安全拦截，且未创建 Bridge 目录。`v7` 已增加 Native 预期数、实际数、缺失数和加盐差异指纹，`v8` 补齐总数不一致但可翻译键缺失为零的显示，`v9` / `v10` 又将复核前失败细分到 Native 阶段。Pixel 最终定位为 `DictionaryAccessor.insertOrUpdate()` 明确拒绝待补词条。`v11` 已在未持久化副本中统计全部拒绝项，并保存加盐拒绝集合指纹；首次真机结果为 303 项，第二次授权复核仍为相同的 303 项，加盐指纹一致且恢复入口已锁定。`v12` 已新增 `SUPPORTED` / `RIME_ONLY` 投影状态，并在第三次探测仍匹配同一批 303 项后完成原事务。Pixel 立即预览和冷启动预览均为五项零变更，证明这些词条保留在 Rime 且不会在每轮重复加入 Google 拼音。真实 Syncthing 双向链路已用隔离合成 peer 完成新增、删除、复活、Bridge 回传和冷启动零变更验收；测试夹具的 `db_name` 不匹配也已通过不含私人内容的阶段分类精确定位并安全拒绝。Pixel 隔离审计包又通过同签名一次性 probe 注入 1 个生成的单字 Native 条目，并以 peer 墓碑触发生产 Bridge 全词典重建；重建后排除项数量和该条目的 tokens、language ID、count、修改标志、规范化标志及 expansion type 均保持不变，清理后排除项和最终同步预览均归零。后续真机预检确认 Native 接受非拼音 language ID 的多字条目和无 tokens 英文条目，但拒绝含非拼音字符的 token；将前两类持久化后再次触发生产重建，排除项数量与覆盖全部 Native 字段的加盐集合指纹均保持一致，清理后恢复为 0。不可投影 Native 条目的运行时守恒至此完成。并发审计随后发现原生 importer 尚未使用 `SaveDictionaryTask.sSaveLock`；`v15` 已将导入的打开、合并、persist 和引擎通知纳入与保存、导出及 Rime Bridge 相同的共享锁。Pixel 已通过真实 Compose 入口覆盖 `backup → Rime`、`Rime → backup` 和 `import → Rime`，备份均完整发布且无 partial，Rime 预览保持五项零变更，同一备份合并前后的 Native 总数和可投影数均为 97,505；任务专用备份 SAF 授权与 Preferences 清理后，Rime 配置仍可用。第一轮规划内存优化已移除 Preview 全量 `EntryPlan`、`TreeSet` 节点、Native source 常驻和 baseline 对象 `HashMap`；第二轮又让轻量 canonicalization 消费临时 Rime snapshot，只保留 canonical key 与 commit 数值。同一 10 万条夹具的有效合成零变更冷启动预览耗时 10.6 秒，峰值 RSS 为 364,212 KiB，结束时 Java Heap PSS 为 44,132 KiB。峰值时 Java Heap PSS 为 44,836 KiB、Native Heap PSS 为 124,672 KiB、总 PSS 为 250,494 KiB；约 328 MiB 的旧首次预览数据来自空 Google Native 词典，不能作为 10 万项 Native 零变更预览的硬门槛。20 万条只读规划预检已通过：Rime 200,000 条、Google Native 约 99,998 条时，真实 Compose 入口在 19.2 秒内得到五项零变更；峰值 Java Heap PSS 为 76,476 KiB、Native Heap PSS 为 102,708 KiB、总 PSS 为 253,615 KiB，30 秒后总 PSS 回落到 219,430 KiB。双引擎新词初始值也已完成隔离候选验收：Weasel 0.17.4／librime 1.13.1 的 `pinyin_simp` 在同步后同时显示 `c=0 d=1e-8 t=0` 夹具和 `c=1` 阳性对照；Pixel 10 Pro 上的全新 release-like 审计包可将 Google `count=1` 生成词条作为首选精确提交，输入会话结束后重新打开 Native 词典得到 `count=2`，证明候选可见性和后续本地学习均成立。夹具与审计包已清理，正式默认输入法已恢复。该结果不证明 Google Native 可持久化 20 万条；后续 Native 扩容必须分阶段验证，不直接进行 20 万条写入或接近 50 万条测试。Pixel 10 Pro 上的独立 SAF 授权丢失验收也已完成：撤销任务专用 URI 授权后，异步 Preview 返回既有的 `ERROR_LOCATION_UNAVAILABLE` 和 `locationAccessible=false`，Compose 显示「目录无法访问，请重新选择」并禁用「立即同步」；重新选择同一目录后授权恢复，Preview 回到五项零变更。全程没有创建事务、访问 Native、发布 Bridge 文件或出现 crash／ANR，隔离包和目录已清理。Bridge 不检测 Syncthing 是否在线，也不等待其他设备完成同步，只处理每轮开始时 SAF 目录中可见的文件。Pixel 10 Pro 已完成两个 `ExternalStorageProvider` 发布中断窗口：缺少正式文件时由 previous 恢复，正式文件已存在时保留 current 并清除 previous；两种状态中的 partial 都被清理，恢复后均只剩单一正式快照且 Preview 为五项零变更。自动同步现纳入本次版本，方案见 [`rime-auto-sync-design.md`](rime-auto-sync-design.md)。既有手动同步证据见 [`rime-dictionary-sync-research.md`](rime-dictionary-sync-research.md)

### Rime 发布前收尾

- [x] 确认本次纳入自动同步：包含删除并在开启时告知，默认 24 小时，范围为 6 小时至 7 天，不要求整点，不检测外部同步软件或文件新旧。调度方案见 [`rime-auto-sync-design.md`](rime-auto-sync-design.md)
- [x] 实现默认关闭的自动同步开关、单选间隔列表和单一 JobScheduler 入口，复用 Rime 队列、事务及完整复核。后台暂停与完成会通知可见页面，返回页面重新读取状态，不增加轮询或自动批准拒绝项
- [x] 完成核心后台闭环：真实 JobService 新增、删除、重新添加，调度更新与取消，安全暂停及界面刷新均已验证。Pixel 隔离词库先完整同步 200 条分散编码样本，再扩展到 240 条，在观测到真实 `PLANNED` 后终止自身进程。系统保留任务，新进程经后台入口完成恢复，没有手动恢复或重置，五项预览为零。临时不完整快照产生解析错误时保留开关与任务，恢复有效文件后后台同步成功。额外冷启动后真实 Compose 预览仍为五项零变更。重试执行机会由系统命令提前触发，不代表验证了自然退避时长或所有事务中断窗口

- [x] 修复用户词库状态页的计数错误：Java 与最终 APK 已改用 `getDictionaryCount()`。Pixel 真实状态接口验证 0、200、240 条，实际 Compose 页面验证中文／英文／合计分别为 240／0／240 和 0／0／0。此前真实用户数据的 533,347 显示是接口误用，不是大规模测试词条注入或容量上限。详见 [`dictionary-health-status-design.md`](dictionary-health-status-design.md)

- [x] 修复数据库名称兼容：配置和 Bridge 发布使用规范名称 `pinyin_simp`，peer 名称按 librime 官方规则去掉最后一个 `.userdb` 及其后缀后匹配。本机带后缀 Bridge 仍须验证标记和 UUID，不重建 Profile 或 baseline。此前只接受无后缀 peer 的限制误拒绝了真实 Rime 快照，现已修正；不同数据库仍被拒绝，peer 文件保持原样。空闲非空 Profile 的真实覆盖升级现已验证，各待处理阶段保留与真实 `PLANNED` 中断恢复分别有证据，不混称逐阶段 APK 升级实测
...
- [x] 本地 API 36、4 KiB 模拟器验证活跃输入的原生重置边界：通知在主线程送达，同一进程与输入连接保持，当前一次输入结束后下一次输入可精确提交新词条。保留原生延迟刷新语义，不强制重置解码器；正常首次引导是测试前置条件
- [x] Pixel 10 Pro 真机完成活跃输入重置边界回归：正常引导完成后，在既有连接内新增词条，主线程通知送达；清空当前测试文本后第二次输入精确提交新词，composing 为 false，IME PID 与连接创建次数保持不变。夹具和测试包已清理，正式默认输入法已恢复
- [x] 首个整合候选 `2.0.10-rime-copywriting-audit.1` 完成 APK 身份、最终 DEX 和运行时契约核对并安装到 Pixel。三个 DEX 与已验收产物逐字节一致，助手未启动、启用或配置新包，旧审计数据与原默认输入法保持不变
- [x] 用户完成首次引导和独立目录配置后，`2.0.10-rime-peer-name-audit.1` 覆盖升级保留配置，真实 Compose 预览得到 Google 新增 97,831、删除 0，Rime 新增 2、删除 0、复活 0，停在确认界面，尚未执行同步
- [x] 用户授权后，从真实 Compose 入口完成一次受控恢复，确认本事务拒绝集合稳定为 303 项，再确认仅保留于 Rime。协调器再次核对集合后完成持久化复核、Bridge 发布及事务提交，即时与冷启动 Preview 均为五项零变更。数量相同不代表与旧审计包是同一集合，日常候选提交、设置与长期学习仍保留独立体验边界
- [x] 清理错误提示中残留的「等待 Syncthing 完成」文案：简体中文和英文的目录检查、词典打开错误改为确认当前目录可访问、文件存在且可读取，不要求外部同步完成。完整重建后已核对最终 APK 中四条文案，资源表不再含 Syncthing 引用，错误码与处理流程不变
- [x] 在真实 SQLite 中验证非空 Profile 重新配置保留：4 个事务阶段的身份、盐对应哈希、generation、baseline 和 pending 数量保持。该实验不等于旧 APK 覆盖升级或各阶段端到端恢复
- [x] 在独立包中从 `4520423` 覆盖升级至 `4520424`：两条 Native 词条、非空 baseline、配置与 Bridge 身份及盐的摘要、Rime 授权、自动开关、39 小时间隔和已有备份均保留。新列表可改选 24 小时
- [x] 完成当前自动入口的有序组合回归：真实备份 → 自动删除一条 → 生产原生导入恢复 → 自动重新添加回传 → 再次备份。Native 数量依次为 2、1、2，最终预览五项为零，两个独立目录可用，两份备份完整且没有 partial。不是强制线程重叠实测
- [ ] 用户已授权正式包名体验，`2.0.11-rc.1`（`4520396`）以原正式签名、非 Debug 状态覆盖安装，安装核对时自动同步关闭。Rime 同步已移到自定义短语上方，旧隔离测试版已按用户后续要求全部卸载，公共目录留给用户手动清理。之后用户已自行配置同步。用户后续已授权集中修复并制作下一版，`2.0.11-rc.2` 已完成同步工程回归，后续状态区布局调整包含在已覆盖安装的 `2.0.11-rc.4`（`4520399`）中，保留数据和默认输入法，等待新版日常体验验收。仍不创建标签或 Release，详见 [第二轮实施记录](rime-rc2-implementation.md)
- [x] 简化首次手动同步：保留首次手动成功后才可开启自动同步的条件，正常路径点击一次后由程序完成，不再要求用户推动中间步骤。用户已接受一次说明规则后，由程序将可安全判定为无法直接转换的条目保留在 Rime，不逐批确认，也不为这些条目补拼音、改编码或增加复杂兼容路径。代码及最新隔离包端到端回归已完成，真正的故障处理和完整持久化复核保持不变，详见第二轮实施记录
- [ ] 优化用户词典状态展示并加入 Rime 信息：分组呈现本机词库与同步状态、最近成功同步、自动同步及仅保留在 Rime 的数量，替换「滚动副本」「恢复旁路」「落盘」等难懂术语，文件细节按需查看。参考 SyncClipboard Kotlin 增加颜色与文字结合的状态指示，自动同步开启后的正常摘要像自动备份一样显示上次成功同步时间，当前异常不能被历史成功时间掩盖。复用现有状态来源，保持只读、按需刷新，核对英文统计与导出数量不一致的口径，不为显示数量扫描整份词库。代码、界面文本、最新状态通知与失败记录已通过工程核对。`rc.3` 已修正整块文字缩进，完整本机参数先于 Rime，默认折叠详情，提供带展开动画的独立按钮和重新检查按钮。`rc.4` 还修复了复查时的高度抖动，删除下方重复的 Rime 状态行，并按自动开关显示折叠摘要。布局、复查坐标及摘要开关关系已通过隔离界面回归，主观视觉与完整 TalkBack 体验仍待用户验收，详见 [状态展示设计](dictionary-health-status-design.md#词库与-rime-同步状态总览)
- [x] 修复目录迁移问题：已在隔离包中复现，根 URI 改变导致程序重建本地身份、盐和基线，与复制过来的本设备快照身份冲突，预览返回合并错误 14。原文件不变时，使用原身份的临时内存合并成功，Native 两条合成记录保持不变。修复需安全延续同一同步关系的身份与状态，不能要求删本设备目录或关闭身份校验。修复已实现，真实 SAF 的两种迁移顺序及后续自动同步已通过首轮回归。最新身份冲突分类、缺失文件时的保留与重试、迁移后自动删除以及修正后的手动同步均已通过设备回归，隔离测试已清理，详见 [实际使用反馈](rime-auto-sync-design.md#实际使用反馈待体验验收后处理)

此前验收叙述以研究文档的「最终一致性审阅」为准：Rime 只验证了候选列表可见性，Google 验证了提交与 count 增长；重命名窗口测试使用人工构造的中断后文件状态，不是运行中杀进程测试。正常 Preview 会读取 Native，仅授权失效请求在 Native 访问前被拦截。
...
- [x] 对照 Gboard 复核固定 120 Hz 请求、触摸 boost、内容速度提示和设备 ARR 能力
- [x] V29 移除固定 120 Hz Window/View vote，API 35+ 改用触摸动态 boost，并在 `onFinishInputView()` 对称释放
