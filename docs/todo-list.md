...
- [ ] 用户已授权正式包名体验，`2.0.11-rc.1`（`4520396`）以原正式签名、非 Debug 状态覆盖安装，安装核对时自动同步关闭。Rime 同步已移到自定义短语上方，旧隔离测试版已按用户后续要求全部卸载，公共目录留给用户手动清理。之后用户已自行配置同步。用户后续已授权集中修复并制作下一版，`2.0.11-rc.2`（`4520397`）已完成最终工程回归并覆盖安装，保留数据和默认输入法，等待新版日常体验验收。仍不创建标签或 Release，详见 [第二轮实施记录](rime-rc2-implementation.md)
