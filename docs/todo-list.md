...
- [ ] 实现默认关闭的自动同步开关、间隔设置和单一 JobScheduler 入口，复用 Rime 队列、事务及完整复核。系统中断和临时失败由程序自动恢复或退避重试，只有无法安全自行解决的问题才要求用户介入，不自动扩大未经授权的拒绝例外集合
- [ ] 完成真实后台入口、自动删除、调度更新与取消、中断后自动收敛、必要的用户处理状态及冷启动闭环验收，再与手动同步统一验收
