...
- [ ] 用户已授权并从真实 Compose 入口执行上述同步，原生写入检查拒绝 303 项，尚未完成。当前停止且未自动重试，下一步需授权复核拒绝集合，再确认是否以 `RIME_ONLY` 完成事务；数量相同不代表与旧包是同一集合。持久化和零变更闭环仍待验收，继续保留日常输入、设置与长期学习的体验边界
...
- [ ] 当前候选已完成重建和静态门禁，实际同步及剩余整合回归通过后，再决定合并和发布
