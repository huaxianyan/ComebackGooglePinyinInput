...
- [ ] 隔离验证最终剪贴板候选细节：V14 已确认视觉/功能理想；比较最大宽度调整为 `200dp + 2 × 21sp em`（默认约 242dp）后的长中文、英文和混合文本，确认多显示约两个中文字且仍不接近关闭键；同时复核图标距左 divider 10dp、距文字 6dp、组合居中、END ellipsis 和回收清理不回退
