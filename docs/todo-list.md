...
- [ ] 隔离验证最终剪贴板候选细节：V13 已确认 ART/功能正常；复核最后 4dp 光学调整后关闭按钮和两侧 divider 不回退、18dp Material paste 图标距左 divider 10dp 且距文字 6dp、28dp start reserve 仍使组合居中并限制在 200dp/END ellipsis 内、回收普通候选时不残留图标或间距、关闭及展开箭头恢复
