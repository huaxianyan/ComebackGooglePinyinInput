...
- [ ] 补齐保存／导入的刷新边界：`70d84c7` 只让 Bridge 在解锁前同步刷新，保存和导入仍异步通知后解锁，下一位读取者可能读旧版本。先让相关写入方复用同步刷新入口，再验证真实交错路径，不把该静态风险描述为已发生的数据丢失
- [ ] 最终整合回归覆盖活跃输入会话，以及备份／导入与新版刷新入口的交错运行，不能用旧版并发验收替代新版证据
