...
- [ ] 用户已授权正式包名体验，`2.0.11-rc.1`（`4520396`）以原正式签名、非 Debug 状态覆盖安装，安装核对时自动同步关闭。Rime 同步已移到自定义短语上方，旧隔离测试版已按用户后续要求全部卸载，公共目录留给用户手动清理。之后用户已自行配置同步。用户后续已授权集中修复并制作下一版，`2.0.11-rc.2` 已完成正式签名构建，因设备断开，最后几项回归与安装待完成。仍不创建标签或 Release，详见 [第二轮实施记录](rime-rc2-implementation.md)
- [ ] 简化首次手动同步：保留首次手动成功后才可开启自动同步的条件，正常路径点击一次后由程序完成，不再要求用户推动中间步骤。用户已接受一次说明规则后，由程序将可安全判定为无法直接转换的条目保留在 Rime，不逐批确认，也不为这些条目补拼音、改编码或增加复杂兼容路径。代码及首轮端到端回归已完成，真正的故障处理和完整持久化复核保持不变，剩余设备复核见第二轮实施记录
- [ ] 优化用户词典状态展示并加入 Rime 信息：分组呈现本机词库与同步状态、最近成功同步、自动同步及仅保留在 Rime 的数量，替换「滚动副本」「恢复旁路」「落盘」等难懂术语，文件细节按需查看。参考 SyncClipboard Kotlin 增加颜色与文字结合的状态指示，自动同步开启后的正常摘要像自动备份一样显示上次成功同步时间，当前异常不能被历史成功时间掩盖。复用现有状态来源，保持只读、按需刷新，核对英文统计与导出数量不一致的口径，不为显示数量扫描整份词库。代码及首轮界面文本核对已完成，最新状态通知、失败记录与主观视觉验收待完成，详见 [状态展示设计](dictionary-health-status-design.md#待实施词库与-rime-同步状态总览)
- [ ] 修复目录迁移问题：已在隔离包中复现，根 URI 改变导致程序重建本地身份、盐和基线，与复制过来的本设备快照身份冲突，预览返回合并错误 14。原文件不变时，使用原身份的临时内存合并成功，Native 两条合成记录保持不变。修复需安全延续同一同步关系的身份与状态，不能要求删本设备目录或关闭身份校验。修复已实现，真实 SAF 的两种迁移顺序及后续自动同步已通过首轮回归。最新身份冲突分类和迁移后自动删除仍待设备复核，隔离测试已清理，详见 [实际使用反馈](rime-auto-sync-design.md#实际使用反馈待体验验收后处理)
