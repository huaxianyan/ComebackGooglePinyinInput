...
- [ ] 修复 Native 同进程复核时序：直接调用生产 `apply()`／`read()` 已复现即时复核失败而新进程能读取持久化结果。引擎通知是异步 `Handler.post()`，不代表重新载入已完成；删除实验在测试专用主线程队列屏障后确认总数为 0。需要可靠的刷新完成契约或独立持久化读取能力，禁止固定延时、引擎重启或移除复核
