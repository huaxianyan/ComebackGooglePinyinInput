...
- [ ] 复现并修复目录迁移问题：用户更换目录并复制原目录内容后，自动同步关闭，要求手动同步，但手动同步持续失败并提示检查其他文件夹，删除本设备文件夹才可继续。目标是合法原有数据无需删本设备目录即可完成同步，具体错误原文与根因尚未核对。先记录，待本轮实际使用验收结束后处理，详见 [实际使用反馈](rime-auto-sync-design.md#实际使用反馈待体验验收后处理)
