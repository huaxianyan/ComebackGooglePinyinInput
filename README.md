...
- 增加本地用户词典自动备份，由用户选择内部存储或本机 SD 卡目录；备份与手动导入共用该目录。
...
在“设置 → 字典”中选择“备份和导入位置（仅本地）”后，自动备份、立即备份、版本轮换和内置手动导入都会使用同一个用户授权目录。目录可位于内部存储或本机 SD 卡；不接受云端 provider。公共目录中的文件不会因清除应用数据或卸载创造性 AI 版而被删除。
